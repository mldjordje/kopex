...
import { deleteProductEntry, getProductById, updateProductEntry } from '@/lib/products';
...
const parseDocuments = (value: FormDataEntryValue | null): ProductDocument[] => {
  if (!value || typeof value !== 'string') {
    return [];
  }
  try {
    ...
    return parsed
      ...
      .filter((item): item is ProductDocument => Boolean(item));
  } catch {
    return [];
  }
};
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const clearGallery = String(formData.get('clearGallery') || '') === '1';
    ...
    const hasExistingGallery = formData.has('existingGallery');
    const hasExistingDocuments = formData.has('existingDocuments');
    const existingGallery = parseStringArray(formData.get('existingGallery'));
    const existingDocuments = parseDocuments(formData.get('existingDocuments'));
    ...
    const [heroUpload, galleryUploads, documentUploads, existingProduct] = await Promise.all([
    ...
    if (!existingProduct) {
      return NextResponse.json({ message: 'Proizvod ne postoji.' }, { status: 404 });
    }

    const baseHeroImage = heroImageUrl || existingProduct?.heroImage || null;
    const baseGallery = hasExistingGallery ? existingGallery : existingProduct.gallery || [];
    const baseDocuments = hasExistingDocuments ? existingDocuments : existingProduct.documents || [];

    const heroImage = heroUpload ?? (clearHero ? null : baseHeroImage);
    const gallery = [
      ...(clearGallery ? [] : baseGallery),
      ...galleryUploads
    ];
    const documents = [
      ...(clearDocuments ? [] : baseDocuments),
      ...documentUploads
    ];

    const resolvedSlug = await updateProductEntry({
      id,
      name,
      slug,
